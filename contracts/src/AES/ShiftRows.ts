...
function shiftRows(state: Byte16): Byte16 {
  ...
  let row4 = Gadgets.rightShift64(
    Gadgets.and(Field(0x00000000ffffff00), state.bot, 64),
    8
  );
  ...
  let row3 = Gadgets.or(
    Gadgets.rightShift64(n8_9, 16),
    Gadgets.leftShift64(nA_B, 16),
    64
  );

  // Row 2: Shift bytes to the left. First byte (n4) cycles over to location n7
  let row2 = Gadgets.leftShift64(
    Gadgets.and(Field(0x0000000000ffffff), state.top, 64),
    8
  );
...
function shiftRowsInv(state: Byte16): Byte16 {

  // Row 2: Shift bytes to the left. First byte (n4) cycles over to location n7
  let row4 = Gadgets.leftShift64(
    Gadgets.and(Field(0x0000000000ffffff), state.bot, 64),
    8
  );
  let n12 = Gadgets.and(Field(0x00000000ff000000), state.bot, 64);
  row4 = Gadgets.or(row4, Gadgets.rightShift64(n12, 24), 64);
  ...
  // Row 4: Shift bytes to the right. Last byte (n15) cycles back to location n12
  let row2 = Gadgets.rightShift64(
    Gadgets.and(Field(0x00000000ffffff00), state.top, 64),
    8
  );
  let n7 = Gadgets.and(Field(0x00000000000000ff), state.top, 64);
  row2 = Gadgets.or(row2, Gadgets.leftShift64(n7, 24), 64);
